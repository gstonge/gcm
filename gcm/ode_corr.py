...
infection function, taking into account two-point correlations
"""
...
def get_state_meta(mmax, nmax, Pm_n, Pn_m):
...
def advance(sm, fni, tvar, inf_mat, state_meta):
...
def vector_field(v, t, inf_mat, state_meta):
    ...
    rm = np.sum(inf_mat[2:,:]*(nmat[2:,:]-imat[2:,:])*fni[2:,:]*\
               Pn_m_mat[:,2:,:]/nmax[2:,:],axis=(1,2))
    rm /= np.sum((nmat[2:,:]-imat[2:,:])*fni[2:,:]*\
               *Pn_m_mat[:,2:,:]/nmax[2:,:],axis=(1,2))
    rhon = np.sum((m-1)*rm*sm*Pm_n,axis=1)
    rhon /= np.sum(sm*Pm_n,axis=1)
    ...
    fni_field[2:,1:nmax+1] += ((nmat[2:,:nmax] - imat[2:,:nmax])
                               *(inf_mat[2:,:nmax] + rhon_mat[2:,:]))*fni[2:,:nmax]
